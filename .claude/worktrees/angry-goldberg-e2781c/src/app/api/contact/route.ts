...
export async function POST(request: Request) {
  try {
    ...
    const { nombre, email, telefono, propuesta, tipo } = body;
    ...
    // Traducción del tipo para el correo
    const tipoMap: Record<string, string> = {
      web: 'App / Sitio Web',
      api: 'API / Backend',
      mobile: 'Móvil / Híbrida',
      other: 'Otro / Consultoría',
    };
    const tipoDisplay = tipo ? (tipoMap[tipo] || tipo) : 'No especificado';

    const htmlBody = `
      <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px;">
        <h2 style="color: #2563eb; border-bottom: 2px solid #2563eb; padding-bottom: 10px;">Nueva Propuesta: ${nombre}</h2>
        ...
        <p style="font-size: 16px;"><strong>🎯 Tipo de Proyecto:</strong> <span style="background: #2563eb; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px;">${tipoDisplay}</span></p>
        ...
        <p style="font-size: 12px; color: #888; text-align: center;">Este correo es confidencial y se generó desde el portal web de GaloDev.</p>
      </div>
    `;
